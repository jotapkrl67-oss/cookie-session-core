...
def test_injected_runtime_is_valid_javascript(tmp_path):
    ...
    script_path.write_text(
        ...
        "globalThis.history={pushState(){},replaceState(){}};" + runtime,
        encoding="utf-8",
    )
...
def test_localstorage_runtime_works_in_transparent_mode_and_syncs_property_writes(tmp_path):
    node = shutil.which("node")
    if not node:
        pytest.skip("Node.js is required for the localStorage runtime check")
    item = ConsumedLaunch(
        **{
            **launch().__dict__,
            "local_storage_items": {"token": "server-value", "": "empty-key-value"},
        }
    )
    html = rewrite_text(
        b"<html><head></head></html>",
    ...
    script_path.write_text(
        ...
        "globalThis.navigator={};globalThis.Element=class Element{};Element.prototype.setAttribute=function(){};"
        "globalThis.Document=class Document{};globalThis.document={readyState:'loading',visibilityState:'visible',"
        "addEventListener(){},querySelectorAll(){return[]},head:{appendChild(){}},documentElement:{appendChild(){}}};"
        "globalThis.history={pushState(){},replaceState(){}};globalThis.addEventListener=function(){};"
        "const calls=[];globalThis.fetch=async function(url,init){calls.push({url,init});"
        "return {ok:true,async json(){return {snapshot:{token:'server-value',theme:'dark','':'empty-key-value'}}}}};"
        + runtime
        + "\nif(localStorage.getItem('token')!=='server-value')throw new Error('seed missing');"
        "const proto=Object.getPrototypeOf(localStorage);"
        "if(proto.getItem.call(localStorage,'token')!=='server-value')throw new Error('prototype get failed');"
        "proto.setItem.call(localStorage,'viaProto','ok');"
        "if(localStorage.getItem('viaProto')!=='ok')throw new Error('prototype set failed');"
        "if(localStorage.getItem('stale')!==null)throw new Error('stale key survived');"
        "if(localStorage.getItem('')!=='empty-key-value')throw new Error('empty key missing');"
        "localStorage.theme='dark';sessionStorage.setItem('temporary','yes');"
        "if(localStorage.theme!=='dark'||!Object.keys(localStorage).includes('theme'))throw new Error('property access failed');"
...
def test_localstorage_sync_endpoint_is_reserved_only_at_proxy_root():
    assert _is_localstorage_sync_path("__localstorage/sync")
    assert _is_localstorage_sync_path("/__localstorage/sync/")
    assert not _is_localstorage_sync_path("api/__localstorage/sync")
    assert not _is_localstorage_sync_path("_host/example.com/__localstorage/sync")
...
def test_chatgpt_profile_menu_is_visibly_locked_and_event_blocked():
    ...
    assert '"profilePrivacy":{"enabled":true' in result
...
def test_profile_lock_policy_is_injected_for_other_products():
    result = rewrite_text(
        b"<html><head></head></html>",
        "text/html",
        current_target="https://app.example.com/",
    ...
    assert '"profilePrivacy":{"enabled":true' in result
    assert "Google Account" in result
