...
class Settings(BaseSettings):
    ...
    lovable_return_url: HttpUrl
    allowed_origins: str
    max_browser_sessions: int = Field(default=3, ge=1, le=20)
    browser_session_ttl_seconds: int = Field(default=1800, ge=60, le=7200)
    ...
    def token_pepper(self) -> bytes:
        return self._decode_secret(
            self.launch_token_pepper_base64, "LAUNCH_TOKEN_PEPPER_BASE64"
        )
