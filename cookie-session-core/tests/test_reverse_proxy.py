...
def test_cookie_matches_by_registrable_domain_fallback():
    item = ConsumedLaunch(
        user_id="u",
        service_id="s",
    ...
    assert "HSID=h" in header
    assert "UNRELATED" not in header


def test_runtime_location_interceptors_are_injected():
    item = launch()
    result = rewrite_text(
        b"<html><head></head></html>",
        "text/html",
    ...
    assert "patch(window.location)" in result
    assert "Document.prototype" in result
    assert "Location.prototype" in result
    assert "descAssign" in result
    assert "descReplace" in result
    assert "descHref" in result
    ...
    assert "setProperty" in result, "CSSStyleDeclaration.setProperty must be intercepted"
