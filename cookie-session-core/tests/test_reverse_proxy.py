...
def test_cookie_and_allowlist_paths_use_rfc_boundaries():
    ...
    restricted = ConsumedLaunch(
        **{**item.__dict__, "allowed_paths": ("/admin",)}
    )
...
def test_cloudflare_challenge_relay_requires_original_public_hostname():
    assert challenge_can_be_relayed(
        "https://app.example.com/account", "https://app.example.com"
    )
    assert not challenge_can_be_relayed(
        "https://app.example.com/account", "https://servico.jbtools.site"
    )
    assert not challenge_can_be_relayed(
        "https://app.example.com/account", "http://app.example.com"
    )
    assert challenge_can_be_relayed(
