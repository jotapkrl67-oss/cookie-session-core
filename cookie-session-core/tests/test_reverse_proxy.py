...
def test_absolute_allowed_subdomain_inside_javascript_stays_in_proxy():
    ...
    assert (
        f'location.href="https://api.jbtools.site{prefix}/_host/'
        'www.kalodata.com/aitools"' in result
    )
...
def test_dreamface_public_cdn_assets_stay_direct_to_avoid_proxy_rate_limit():
    ...
    url = (
        "https://cdnseo.dreamfaceapp.com/seo/static_resource/prod/"
        "_nuxt/VN2cyalJ.js"
    )
    assert browser_url(
        url,
...
def test_cloudflare_insights_stays_direct_and_sri_is_removed_from_proxied_html():
    ...
    html = (
        f'<html><head><script src="{url}" integrity="sha512-original"></script>'
        '</head></html>'
    ).encode()
    result = rewrite_text(
        html,
...
def test_nuxt_router_base_uses_proxy_prefix_without_location_monkeypatch():
    ...
    prefix = f"/proxy/{item.service_id}"
    html = (
        '<html><head><script>window.__NUXT__={config:{app:{baseURL:"/"}}};'
        '</script></head></html>'
    ).encode()
    result = rewrite_text(
        html,
