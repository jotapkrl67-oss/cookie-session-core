...
def test_host_only_import_cookies_follow_same_site_service_hosts_only():
    base = launch()
    item = ConsumedLaunch(
        **{
            **base.__dict__,
    ...
    assert _cookie_header(item, "https://account.example.com/profile") == (
        "host_login=credential"
    )
    assert _cookie_header(item, "https://unrelated.test/profile") == ""


def test_google_host_only_login_reaches_same_site_realtime_endpoint():
    base = launch()
    item = ConsumedLaunch(
        **{
            **base.__dict__,
    ...
    assert _cookie_header(item, target) == "SAPISID=login"
...
def test_upstream_request_context_preserves_source_host_across_proxy_hosts():
    item = launch()
    prefix = f"/proxy/{item.service_id}"
    assert upstream_referer(
        f"https://servico.jbtools.site{prefix}/account/page?q=1",
        launch=item,
        proxy_prefix=prefix,
    ) == "https://app.example.com/account/page?q=1"
    assert upstream_referer(
        f"https://servico.jbtools.site{prefix}/_host/cdn.example.com/frame",
        launch=item,
        proxy_prefix=prefix,
    ) == "https://cdn.example.com/frame"
    assert fetch_site(
        "https://gemini.google.com/app", "https://signaler-pa.clients6.google.com/api"
    ) == "same-site"
    assert fetch_site("https://app.example.com/a", "https://app.example.com/b") == (
        "same-origin"
    )
    assert fetch_site("https://app.example.com", "https://cdn.other.test") == "cross-site"
...
def test_runtime_maps_request_objects_and_csp_allows_challenge_widgets():
    ...
    assert 'currentTarget":"https://app.example.com/' in result
    assert "HTMLImageElement,'srcset'" in result
