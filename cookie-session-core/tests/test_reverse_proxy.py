...
def test_google_login_cookies_from_header_import_reach_google_sibling_host_only():
    base = launch()
    item = ConsumedLaunch(
        **{
            **base.__dict__,
            "upstream_url": "https://gemini.google.com/app",
    ...
    google_target = "https://signaler-pa.clients6.google.com/punctual/v1/chooseServer"
    assert _cookie_header(item, google_target) == "SAPISID=login"
    assert _cookie_header(item, "https://cdn.other.test/file") == ""
    assert "theme=dark" not in _cookie_header(item, google_target)


def test_direct_google_cookie_wins_over_compatibility_fallback():
    base = launch()
    item = ConsumedLaunch(
        **{
            **base.__dict__,
    ...
    target = "https://signaler-pa.clients6.google.com/punctual/v1/chooseServer"
    assert _cookie_header(item, target) == "SAPISID=domain-cookie"
...
def test_javascript_absolute_same_site_subdomain_stays_inside_proxy():
    item = launch()
    result = rewrite_text(
        b'{"pricing":"https://account.example.com/pricing"}',
        "application/json",
    ...
    assert result == (
        '{"pricing":"https://servico.jbtools.site'
        f'/proxy/{item.service_id}/_host/account.example.com/pricing"}}'
    )


def test_kalodata_javascript_hard_navigation_keeps_proxy_prefix():
    base = launch()
    item = ConsumedLaunch(
        **{
            **base.__dict__,
            "upstream_url": "https://www.kalodata.com/explore",
            "allowed_domains": ("kalodata.com",),
        }
    )
    prefix = f"/proxy/{item.service_id}"
    result = rewrite_text(
        b'location.href="/pricing";const account="/account";const empty="";',
        "application/javascript",
        current_target="https://www.kalodata.com/explore",
        launch=item,
        proxy_prefix=prefix,
        public_base_url="https://servico.jbtools.site",
    ).decode()
    assert f'location.href="{prefix}/pricing"' in result
    assert f'account="{prefix}/account"' in result
    assert 'empty=""' in result


def test_root_literal_rewrite_is_not_applied_to_unrelated_services():
    item = launch()
    result = rewrite_text(
        b'location.href="/pricing"',
        "application/javascript",
        current_target="https://app.example.com/explore",
        launch=item,
        proxy_prefix=f"/proxy/{item.service_id}",
    ...
    assert result == 'location.href="/pricing"'
...
def test_runtime_maps_request_objects_and_csp_allows_challenge_widgets():
    ...
    assert "getRegistrations" in result
    assert "r.unregister()" in result
