...
O Railway executará FastAPI, Chromium e WebSocket em um único container.
...
Para começar, use memória suficiente para Chromium. Recomenda-se cerca de 2 GB e:

```text
MAX_BROWSER_SESSIONS=3
```

Se houver encerramentos por falta de memória, diminua para 1 ou 2 antes de aumentar
o plano.
...
PUBLIC_BASE_URL=https://SEU_SERVICO.up.railway.app
LOVABLE_RETURN_URL=https://SEU_PROJETO_LOVABLE
...
Se `browser` falhar, verifique memória e logs do build.
...
4. navegue para `launch_url`.

Ao clicar em Fechar na sessão remota, o usuário volta para `LOVABLE_RETURN_URL`.
...
4. usuário atribuído lista o perfil;
5. outro usuário não lista o perfil;
6. launch token abre uma vez;
7. repetir o token falha;
8. contexto fecha ao usar Fechar;
9. logs não contêm body, Authorization ou cookies.

Somente depois faça um teste manual com uma conta autorizada.
