...
from cookie_session_core.schemas import ServiceInput
...
def test_vault_is_bound_to_user_service_and_name():
    ...
    assert vault.decrypt(
        encrypted.ciphertext, encrypted.nonce, "user-a", "service-a", "session"
    ) == "secret"
    with pytest.raises(InvalidTag):
        vault.decrypt(
            encrypted.ciphertext, encrypted.nonce, "user-b", "service-a", "session"
        )
...
def test_json_same_site_is_normalized_for_playwright():
    cookie = parse_cookie_import(
        '[{"name":"session","value":"fake","domain":"example.com",'
