...
def create_app() -> FastAPI:
    ...
    async def launch_service(
        ...
        upstream_path = (
            urlparse(
                await request.app.state.pool.fetchval(
                    "SELECT upstream_url FROM cookie_core_services WHERE id=$1", service_id
                )
            ).path
            or "/"
        )
        ...
        return {
            "launch_url": f"{base}/proxy/{service_id}{upstream_path}?{query}",
            "expires_in": 30,
        }
