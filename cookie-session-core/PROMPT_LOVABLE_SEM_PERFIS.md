...
9. no cadastro da ferramenta, exponha `proxy_hostname` como campo opcional
   "Subdomínio transparente"; envie somente o hostname, sem `https://` ou caminho.
...
- Não envie profileId.
- launch_url pode apontar para `https://servico.jbtools.site/proxy/...` ou para o
  `proxy_hostname` dedicado retornado pela API; não monte essa URL no frontend.
