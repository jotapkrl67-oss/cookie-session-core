...
def parse_localstorage_import(raw: str) -> list[ImportedLocalStorageItem]:
    if not raw.strip() or len(raw.encode()) > 500_000:
    ...
    for key, value in decoded.items():
        if not isinstance(key, str) or not key.strip():
            raise ValueError("LocalStorage key must be a non-empty string")
        if len(key) > 500:
            raise ValueError(f"LocalStorage key too long: {len(key)}")
        str_value = value if isinstance(value, str) else json.dumps(value, ensure_ascii=False)
        if len(str_value.encode()) > 512_000:
            raise ValueError(f"LocalStorage value too large for key: {key}")
        if any(ord(char) < 32 and char not in "\t\n\r" for char in key):
            raise ValueError(f"LocalStorage key contains control characters: {key}")
        output.append(ImportedLocalStorageItem(key=key.strip(), value=str_value))
    if not output:
