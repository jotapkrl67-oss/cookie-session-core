...
def extract_challenge_info(url: str, status: int, body: bytes, headers: Any) -> CfChallengeInfo:
    ...
    ray_id: str | None = None
    hdr_ray = getattr(headers, "get", lambda _k: None)("cf-ray") if hasattr(headers, "get") else None
    if hdr_ray:
...
class _BaseApiSolver:
    ...
    async def _poll_until(
        ...
        *,
        ...
        try:
            resp = await self._http.post(create_url, json=create_payload, timeout=min(30.0, timeout))
        ...
        if not data:
            return CfSolveResult(success=False, provider=self.provider, error="empty create response")
        ...
        while time.monotonic() < deadline:
            ...
            if status in {"ready", "success"}:
                ...
                for c in cookies:
                    if isinstance(c, dict) and str(c.get("name", "")).lower() == "cf_clearance":
                        ...
                        break
                if not cf_clearance:
                    user_agent_data = solution.get("userAgent") or solution.get("userAgentData")
                    cf_clearance = (
                        solution.get("cf_clearance")
                        or last_result.get("cf_clearance")
                        or None
                    )
                turnstile = (
                    solution.get("token")
            ...
            if status in {"processing", "idle", "waiting", "queued"}:
                await asyncio.sleep(poll_interval)
                continue
            err = last_result.get("errorDescription") or last_result.get("error") or str(last_result)
            return CfSolveResult(success=False, provider=self.provider, error=f"solve failed: {err}")
        return CfSolveResult(
            success=False,
            provider=self.provider,
...
class TwoCaptcha(_BaseApiSolver):
    ...
    async def solve(self, info: CfChallengeInfo, *, timeout: float) -> CfSolveResult:
        ...
        while time.monotonic() < deadline:
            ...
            if status in (0, "0") and str(result.get("request", "")) not in {"CAPCHA_NOT_READY", "CAPTCHA_NOT_READY"}:
...
class CustomEndpointSolver(_BaseApiSolver):
    ...
    async def solve(self, info: CfChallengeInfo, *, timeout: float) -> CfSolveResult:
        ...
        if isinstance(data, dict) and (data.get("success") or data.get("status") in {"ready", "success", 1}):
...
class CloudflareSolverOrchestrator:
    ...
    def __init__(self, settings: Settings, clearance_cache: Any):
        ...
        self._solver: CfChallengeSolver | None = None
        self._solver_lock = asyncio.Lock()

    async def aclose(self) -> None:
        async with self._solver_lock:
            if self._solver is not None and hasattr(self._solver, "aclose"):
                with _suppress_ctx(Exception):
                    await self._solver.aclose()
            self._solver = None

    async def _get_solver(self) -> CfChallengeSolver | None:
        provider = self.settings.cf_solver_provider
        if provider == CfSolverProvider.NONE:
            return None
        if not self.settings.cf_solver_api_key and provider != CfSolverProvider.CUSTOM:
            logger.warning(
        ...
        async with self._solver_lock:
            if self._solver is None:
                ...
                try:
                    self._solver = ctor(
                        api_key=self.settings.cf_solver_api_key,
                        endpoint=endpoint,
                    )
                except Exception as exc:
    ...
    async def try_solve(
        ...
        logger.info(
            "Starting Cloudflare solve domain=%s sitekey=%s ray=%s provider=%s",
            info.domain,
            info.sitekey,
            info.ray_id,
            self.settings.cf_solver_provider.value,
        )
        last: CfSolveResult | None = None
        for attempt in range(1, max(1, self.settings.cf_solver_max_retries) + 1):
            last = await solver.solve(
                info, timeout=float(self.settings.cf_solver_timeout_seconds)
            )
            last.log_summary()
            if last.success:
                if last.cf_clearance and info.domain:
                    ...
                    logger.info(
                        "cf_clearance injected into cache domain=%s ttl=%ss attempt=%s",
                        info.domain,
                        ttl,
                        attempt,
                    )
                return last
            if attempt < self.settings.cf_solver_max_retries:
