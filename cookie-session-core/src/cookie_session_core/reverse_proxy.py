...
def escaped_proxy_service_id(
    path: str, browser_referer: str | None, browser_cookies: Mapping[str, str]
) -> str | None:
    """Recover framework/Cloudflare root URLs that escaped a path proxy.

    Some third-party loaders resolve ``/cdn-cgi`` or ``/public`` with browser
    internals that cannot be monkey-patched. The Referer identifies the path
    proxy that owns the request and the per-service root cookie proves that its
    grant was issued to this browser.
    """
    if not any(path == root.rstrip("/") or path.startswith(root) for root in ROOT_ESCAPE_PATHS):
        return None
    if not browser_referer:
        return None
    referer_path = urlparse(browser_referer).path
...
def _runtime_script(launch: ConsumedLaunch, prefix: str, public_base_url: str) -> str:
    ...
    return (
        ...
        "catch(_e){}"
        "(function patchGoogleGSI(){"
        "let initialized=false;"
        "function installId(){const ga=window.google&&window.google.accounts&&window.google.accounts.id;"
        "if(!ga||ga.__cookieCorePatched)return;ga.__cookieCorePatched=true;"
        "const oi=ga.initialize.bind(ga);ga.initialize=function patchedInitialize(config){try{"
        "if(config&&typeof config==='object'&&!config.__cookieCoreOriginOverride){"
        "const oc=Object.assign({},config);try{Object.defineProperty(oc,'__cookieCoreOriginOverride',{value:true});}"
        "catch(_e){}oc.client_id&&(delete oc.client_id);oc.login_uri&&(delete oc.login_uri);"
        "oc.cancel_on_tap_outside=true;oc.use_fedcm_for_prompt=undefined;oc.ux_mode='popup';"
        "initialized?console.warn('[CookieCore] GSI initialize suppressed (origin mismatch, already handled)'):oi(oc);"
        "initialized=true;return;}return oi(config);}catch(e){console.warn('[CookieCore] GSI patch error:',e);};"
        "ga.initialize.__orig=oi;};"
        "if(window.google&&window.google.accounts&&window.google.accounts.id){installId();return;}"
        "try{const gads=Object.getOwnPropertyDescriptor(window,'google');"
        "if(gads&&gads.configurable){let val=gads.value;"
        "Object.defineProperty(window,'google',{configurable:true,enumerable:true,get(){return val;},"
        "set(v){val=v;setTimeout(function(){if(val&&val.accounts&&val.accounts.id)installId();},0);}});}}"
        "catch(_e){}let tries=0;const iv=setInterval(function(){tries++;if(window.google&&window.google.accounts&&window.google.accounts.id){installId();clearInterval(iv);}"
        "else if(tries>100){clearInterval(iv);}},200);})();"
