...
DIRECT_PUBLIC_ASSET_HOSTS = frozenset({"cdnseo.dreamfaceapp.com"})
...
ROOT_ASSET_STRING = re.compile(
    r"(?P<quote>['\"`])(?P<url>/(?:_next|assets|static|cdn-cgi)/[^'\"`\\\s]*)",
    re.IGNORECASE,
)
...
def browser_url(
    ...
    *,
    ...
    if host in DIRECT_PUBLIC_ASSET_HOSTS:
        return absolute
...
def rewrite_text(
    ...
    *,
    ...
    def replace_allowed_origin(item: re.Match, *, websocket: bool = False) -> str:
        ...
        if actual_host in DIRECT_PUBLIC_ASSET_HOSTS:
            return item.group(0)
