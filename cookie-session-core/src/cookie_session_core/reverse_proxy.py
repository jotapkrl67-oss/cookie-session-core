...
def _cookie_header(launch: ConsumedLaunch, target: str) -> str:
    ...
    for cookie in launch.cookies:
        ...
        matches_target = host == domain or host.endswith("." + domain)
        # A plain `Cookie:` import has no Domain metadata, so the parser has to
        # attach every value to the service's initial hostname. Modern apps use
        # several sibling hosts (account/API/realtime). Without this same-site
        # fallback the initial page is authenticated while those calls receive
        # no login credential at all. `_host_allowed` already treats the same
        # registrable site as one service boundary; never cross that boundary.
        matches_import_host = main_host == domain or main_host.endswith("." + domain)
        same_site_fallback = (
            host != main_host
            and _registrable_domain(host) == _registrable_domain(main_host)
            and matches_import_host
        )
        if not matches_target and not same_site_fallback:
            continue
...
def upstream_referer(
    browser_referer: str,
    *,
    launch: ConsumedLaunch,
    proxy_prefix: str,
) -> str | None:
    """Recover the real upstream Referer hidden behind the proxy URL."""
    referer = urlparse(browser_referer)
    marker = proxy_prefix + "/"
    if not referer.path.startswith(marker):
        return None
    with suppress(HTTPException):
        return resolve_target(launch, referer.path[len(marker) :], referer.query)
    return None


def fetch_site(source: str, target: str) -> str:
    """Return the Fetch Metadata relationship between two upstream URLs."""
    ...
    if (
        ...
        return "same-origin"
    if _registrable_domain(source_parts.hostname or "") == _registrable_domain(
...
if((C.prefix&&v.startsWith(C.prefix))||(!C.prefix&&v.startsWith('/_host/')))return v;
try{{const explicit=/^[a-z][a-z0-9+.-]*:/i.test(v)||v.startsWith('//');
const u=new URL(v,explicit?location.href:C.currentTarget);if(u.origin===location.origin){{
...
if(navigator.serviceWorker){{const sr=navigator.serviceWorker.register.bind(navigator.serviceWorker);
navigator.serviceWorker.register=(u,o)=>sr(map(u),o&&o.scope?{{...o,scope:map(o.scope)}}:o);}}
const sa=Element.prototype.setAttribute;
Element.prototype.setAttribute=function(n,v){{
if(/^(?:href|src|action|poster|data|formaction)$/i.test(n))v=map(v);
else if(/^srcset$/i.test(n)&&typeof v==='string')v=v.split(',').map(x=>{{const p=x.trim().split(/\\s+/);return [map(p[0]),...p.slice(1)].join(' ')}}).join(', ');
...
[HTMLVideoElement,'src'],[HTMLAudioElement,'src'],[HTMLImageElement,'srcset'],
...
enumerable:d.enumerable,get:d.get,set(v){{d.set.call(this,prop==='srcset'&&typeof v==='string'?v.split(',').map(x=>{{const p=x.trim().split(/\\s+/);return [map(p[0]),...p.slice(1)].join(' ')}}).join(', '):map(v))}}}});}}
...
def rewrite_text(
    ...
    *,
    ...
    if content_type.lower().startswith("text/html"):
        script = _runtime_script(launch, proxy_prefix, public_base_url, current_target)
...
class ReverseProxy:
    ...
    async def http(
        ...
        source_target = launch.upstream_url
        if browser_referer := request.headers.get("referer"):
            if recovered_referer := upstream_referer(
                browser_referer, launch=launch, proxy_prefix=prefix
            ):
                source_target = recovered_referer
                headers["Referer"] = recovered_referer
        if request.headers.get("origin"):
            source_parts = urlparse(source_target)
            headers["Origin"] = urlunparse(
                (source_parts.scheme, source_parts.netloc, "", "", "", "")
            )
        if request.headers.get("sec-fetch-site"):
            _replace_response_header(
                headers, "Sec-Fetch-Site", fetch_site(source_target, target)
            )
