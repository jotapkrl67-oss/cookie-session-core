...
def challenge_can_be_relayed(
    ...
    """Allow path-preserving hosts, or the stricter same-origin path mode."""
    if transparent:
        return True
...
def cloudflare_interstitial_response(ray_id: str | None = None) -> Response:
    ...
    detail = (
        ...
        "não permite servir uma Challenge Page emitida para outro domínio. Publique esta "
        "rota no mesmo hostname do upstream, ou isente o IP de saída do proxy no WAF/use "
        "autenticação máquina-a-máquina (Cloudflare Access)."
    )
    headers = {"X-Cookie-Core-Upstream-Challenge": "cloudflare"}
    if ray_id:
        headers["X-Cookie-Core-Cf-Ray"] = ray_id
    return Response(
        json.dumps({"detail": detail, "provider": "cloudflare", "ray_id": ray_id}),
        status_code=502,
        headers=headers,
