...
def _cookie_header(launch: ConsumedLaunch, target: str) -> str:
    ...
    direct_names = {
        ...
        if (
            host == str(cookie["domain"]).lower().lstrip(".")
            or host.endswith("." + str(cookie["domain"]).lower().lstrip("."))
        )
        and path_matches(str(cookie.get("path") or "/"))
    }
    for cookie in launch.cookies:
        domain = str(cookie["domain"]).lower().lstrip(".")
        direct_match = host == domain or host.endswith("." + domain)
        # Cookie-header imports do not carry Domain metadata. Limit the
        # compatibility fallback to Google's known login-cookie family and to
        # sibling *.google.com hosts. A directly matching cookie always wins.
        google_auth_fallback = (
            not direct_match
            and _registrable_domain(host) == "google.com"
            and _registrable_domain(main_host) == "google.com"
            and (main_host == domain or main_host.endswith("." + domain))
            and str(cookie["name"]) in GOOGLE_AUTH_COOKIE_NAMES
            and str(cookie["name"]) not in direct_names
        )
        if not direct_match and not google_auth_fallback:
            continue
...
// Remove service workers that a previous proxy runtime could have registered
// under this service path. They outlive a deploy and can keep serving the broken
// runtime even after the server-side rollback.
if(navigator.serviceWorker&&navigator.serviceWorker.getRegistrations){{
navigator.serviceWorker.getRegistrations().then(rs=>rs.forEach(r=>{{
try{{const p=new URL(r.scope).pathname;if(!C.prefix||p.startsWith(C.prefix+'/'))r.unregister()}}
catch(_e){{}}}})).catch(()=>{{}});}}
...
def rewrite_text(
    ...
    *,
    ...
    # Kalodata uses hard navigations such as location.href="/pricing" in its
    # client bundle. Location.href cannot be safely monkey-patched, so map only
    # this site's quoted root-relative URL literals while rewriting its text
    # assets. Other services retain the established generic behavior.
    if _registrable_domain(urlparse(launch.upstream_url).hostname or "") == "kalodata.com":
        text = QUOTED_ROOT_URL.sub(
            lambda item: (
                item.group("quote")
                + replace_url(item.group("url"))
                + item.group("quote")
            ),
            text,
        )
    ...
    # Match allowed subdomains as well as the exact configured domain. Otherwise
    # an app configured for `kalodata.com` can navigate to `www.kalodata.com`
    # outside the proxy and immediately lose its proxy-held session.
    for allowed in sorted(launch.allowed_domains, key=len, reverse=True):
        host = allowed.lower().lstrip(".")
        rewrite_site = _registrable_domain(host)

        def replace_origin(match: re.Match) -> str:
            matched_origin = match.group(0)
            matched_host = (urlparse(matched_origin).hostname or "").lower()
            main = (urlparse(launch.upstream_url).hostname or "").lower()
            host_part = "" if matched_host == main else f"/_host/{matched_host}"
            return f"{public_base_url.rstrip('/')}{proxy_prefix}{host_part}"

        text = re.sub(
            rf"https?://(?:[a-z0-9-]+\.)*{re.escape(rewrite_site)}(?::\d+)?(?=[/\"'])",
            replace_origin,
            text,
            flags=re.I,
        )

        def replace_ws_origin(match: re.Match) -> str:
            replacement = replace_origin(match)
            return replacement.replace("https://", "wss://").replace("http://", "ws://")

        text = re.sub(
            rf"wss?://(?:[a-z0-9-]+\.)*{re.escape(rewrite_site)}(?::\d+)?(?=[/\"'])",
            replace_ws_origin,
            text,
            flags=re.I,
        )
