...
def _runtime_script(launch: ConsumedLaunch, prefix: str, public_base_url: str) -> str:
    ...
    return (
        ...
        "try{"
        "let sd=Object.getOwnPropertyDescriptor(ElementCSSInlineStyle.prototype,'style');"
        "if(!sd&&window.HTMLElement)sd=Object.getOwnPropertyDescriptor(HTMLElement.prototype,'style');"
        "if(sd&&sd.get){"
        "Object.defineProperty(HTMLElement.prototype,'style',{configurable:sd.configurable,enumerable:sd.enumerable,"
        "get:function(){const s=sd.get.call(this);if(s&&!s.__ccPatched){"
        "try{s.__ccPatched=true;"
        "let ct=Object.getOwnPropertyDescriptor(CSSStyleDeclaration.prototype,'cssText');"
        "if(ct&&ct.set&&ct.get){"
        "Object.defineProperty(s,'cssText',{configurable:true,enumerable:true,"
        "get:function(){return ct.get.call(this)},"
        "set:function(v){ct.set.call(this,rewriteCss(v))}});}"
        "const sp=s.setProperty.bind(s);s.setProperty=function(p,v,...r){return sp(p,rewriteCss(String(v)),...r)};"
        "const urlProps=['backgroundImage','listStyleImage','borderImageSource','borderImage','cursor','maskImage','webkitMaskImage'];"
        "urlProps.forEach(function(p){"
        "try{let pd=Object.getOwnPropertyDescriptor(CSSStyleDeclaration.prototype,p);"
        "if(pd&&pd.set&&pd.get)Object.defineProperty(s,p,{configurable:true,enumerable:true,"
        "get:function(){return pd.get.call(this)},set:function(v){pd.set.call(this,rewriteCss(String(v)))}});}catch(_e){}"
        "});}catch(_e){}return s;}});}"
        "}catch(_e){}"
