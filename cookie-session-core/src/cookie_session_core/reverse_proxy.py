...
def _runtime_script(launch: ConsumedLaunch, prefix: str, public_base_url: str) -> str:
    ...
    return (
        ...
        "if(C.prefix&&window.Storage){try{const S=Storage.prototype,p=C.cookieNamespace+'store:';"
        ...
        "const lsUpserts={},lsDeletes={};let lsFlushTimer=0,lsFlushing=false;"
        "const lsSyncUrl=function(){try{return map(C.prefix+'/__localstorage/sync')}catch(_e){return C.prefix+'/__localstorage/sync'}};"
        "const lsInitRaw=function(){const seed=C.localStorage&&typeof C.localStorage==='object'?C.localStorage:{};"
        "for(const k in seed){if(Object.prototype.hasOwnProperty.call(seed,k)){try{si.call(localStorage,p+k,String(seed[k]))}catch(_e){}}}};"
        "if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',lsInitRaw,{once:true});else setTimeout(lsInitRaw,0);"
        "const lsApplySnapshot=function(snap){if(!snap||typeof snap!=='object')return;"
        "const seen=new Set();for(const k in snap){if(!Object.prototype.hasOwnProperty.call(snap,k))continue;seen.add(p+k);"
        "const v=String(snap[k]);const cur=gi.call(localStorage,p+k);if(cur!==v){try{si.call(localStorage,p+k,v)}catch(_e){}}}"
        "const existing=[];for(let i=0;i<(ln?ln.call(localStorage):0);i++){const k=ki.call(localStorage,i);if(k&&k.startsWith(p))existing.push(k)}"
        "for(const k of existing){if(!seen.has(k)){try{ri.call(localStorage,k)}catch(_e){}}}};"
        "const lsFlush=function(){if(lsFlushing)return;const ups=lsUpserts,del=lsDeletes;"
        "const hasUps=Object.keys(ups).length>0,hasDel=Object.keys(del).length>0;if(!hasUps&&!hasDel)return;"
        "for(const k in ups)delete lsUpserts[k];for(const k in del)delete lsDeletes[k];"
        "lsFlushing=true;const body=JSON.stringify({upserts:hasUps?ups:undefined,deletes:hasDel?Object.keys(del):undefined});"
        "const f=window.fetch;let done=false;const finalize=function(snap){lsFlushing=false;if(snap)lsApplySnapshot(snap)};"
        "try{if(f){f.call(window,lsSyncUrl(),{method:'POST',credentials:'same-origin',"
        "headers:{'Content-Type':'application/json'},cache:'no-store',body:body}).then(function(r){if(!r.ok)throw new Error();return r.json()})"
        ".then(function(d){finalize(d&&d.snapshot);done=true}).catch(function(){lsFlushing=false;"
        "for(const k in ups){if(!(k in lsUpserts))lsUpserts[k]=ups[k]};for(const k in del)lsDeletes[k]=true})}else{"
        "const X=window.XMLHttpRequest;if(X){const x=new X();x.open('POST',lsSyncUrl());x.setRequestHeader('Content-Type','application/json');"
        "x.onload=function(){if(x.status>=200&&x.status<300){try{const d=JSON.parse(x.responseText);finalize(d&&d.snapshot);done=true;"
        "}catch(_e){lsFlushing=false}}else lsFlushing=false};x.onerror=function(){lsFlushing=false;"
        "for(const k in ups){if(!(k in lsUpserts))lsUpserts[k]=ups[k]};for(const k in del)lsDeletes[k]=true};x.send(body)}}}"
        "catch(_e){lsFlushing=false}if(!done&&!f){lsFlushing=false}};"
        "const lsScheduleFlush=function(){if(lsFlushTimer)return;lsFlushTimer=setTimeout(function(){lsFlushTimer=0;lsFlush()},3000)};"
        "addEventListener('beforeunload',lsFlush,{capture:true,passive:true});"
        "addEventListener('pagehide',lsFlush,{capture:true,passive:true});"
        "document.addEventListener('visibilitychange',function(){if(document.visibilityState==='hidden')lsFlush()},{capture:true,passive:true});"
        "S.getItem=function(k){return gi.call(this,p+k)};"
        "S.setItem=function(k,v){k=String(k);lsUpserts[k]=String(v);delete lsDeletes[k];lsScheduleFlush();return si.call(this,p+k,v)};"
        "S.removeItem=function(k){k=String(k);lsDeletes[k]=true;delete lsUpserts[k];lsScheduleFlush();return ri.call(this,p+k)};"
        "S.clear=function(){const a=[];for(let i=0;i<(ln?ln.call(this):0);i++){"
        "const k=ki.call(this,i);if(k&&k.startsWith(p)){a.push(k);lsDeletes[k.slice(p.length)]=true;delete lsUpserts[k.slice(p.length)]}}"
        "if(a.length)lsScheduleFlush();a.forEach(k=>ri.call(this,k))};"
        "S.key=function(n){let j=0;for(let i=0;i<(ln?ln.call(this):0);i++){"
        "const k=ki.call(this,i);if(k&&k.startsWith(p)){if(j++===n)return k.slice(p.length)}}return null};"
        "if(ld&&ld.configurable)Object.defineProperty(S,'length',{configurable:true,"
        "enumerable:ld.enumerable,get:function(){let n=0;for(let i=0;i<ln.call(this);i++){"
        "const k=ki.call(this,i);if(k&&k.startsWith(p))n++}return n}});"
...
class ReverseProxy:
    ...
    async def http(
        ...
        *,
        ...
        if path.rstrip("/").endswith("/__localstorage/sync"):
