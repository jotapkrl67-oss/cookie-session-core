...
class BrowserLikeClient:
    ...
    async def _send_once(
        ...
        *,
        ...
        use_stream = stream and _is_sse_request(request)
        if use_stream:
    ...
    async def send(
        ...
        **kwargs: Any,
        ...
        for attempt in range(max_attempts):
            ...
            body_for_check = None if is_sse else (resp.content or b"")
            ...
            if is_sse and attempt < max_attempts - 1:
                await resp.aclose()
                ...
                if solve_result is not None and solve_result.success:
                    if solve_result.cf_clearance or self.clearance.get(domain):
                        ...
                        try:
                            ...
                            body2 = None if is_sse else (resp2.content or b"")
                            still_blocked = (
        ...
        body_last = None if is_sse else (last_resp.content or b"")
