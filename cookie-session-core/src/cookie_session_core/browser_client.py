...
def _looks_like_cloudflare_response(status: int, headers: Any) -> bool:
    """Cheap heuristic used for Retry-After + fingerprint rotation even when
    the strict challenge/interstitial detection above returns False.
    """
    if status not in (403, 429, 503, 502, 504):
        return False
    return _has_any_cf_header(headers)
    ...
    async def send(
        ...
        **kwargs: Any,
        ...
        for attempt in range(max_attempts):
            ...
            is_cf_429 = (
                status == 429
                and _looks_like_cloudflare_response(status, resp.headers)
            )
            if not is_challenge and not is_cf_429:
                return resp
            ...
            if should_solve and solver is not None:
                ...
                if solve_result is not None and solve_result.success:
                    if solve_result.cf_clearance or self.clearance.get(domain):
                        ...
                        still_blocked = _looks_like_cloudflare_challenge(
                            status2, body2, resp2.headers
                        ) or (
                            status2 == 429
                            and _looks_like_cloudflare_response(status2, resp2.headers)
                        )
        ...
        if _looks_like_cloudflare_challenge(
            ...
            status == 429
            and _looks_like_cloudflare_response(status, last_resp.headers)
        ):
            logger.info(
                "Cloudflare challenge/429 persists on %s after %s attempt(s) impersonate=%s. "
