...
-- Older parser versions interpreted the common JSON session-cookie sentinel
-- `expires: -1` as 1969-12-31, so valid authentication cookies were silently
-- excluded by every active-cookie query. No usable persistent cookie can have
-- an expiry at/before the Unix epoch; recover those rows as session cookies.
UPDATE cookie_core_stored_cookies
SET expires_at=NULL, updated_at=now()
WHERE expires_at IS NOT NULL
  AND expires_at <= TIMESTAMPTZ '1970-01-01 00:00:00+00';
...
CREATE TABLE IF NOT EXISTS cookie_core_stored_localstorage (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id text NOT NULL,
  service_id uuid NOT NULL REFERENCES cookie_core_services(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS cookie_core_stored_localstorage_scope_key
  ON cookie_core_stored_localstorage(user_id,service_id,item_key);
CREATE INDEX IF NOT EXISTS cookie_core_stored_localstorage_owner_idx
  ON cookie_core_stored_localstorage(user_id,service_id) WHERE revoked_at IS NULL;
ALTER TABLE cookie_core_stored_localstorage ENABLE ROW LEVEL SECURITY;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'anon') THEN
    REVOKE ALL ON cookie_core_services, cookie_core_stored_cookies,
      cookie_core_stored_localstorage,
