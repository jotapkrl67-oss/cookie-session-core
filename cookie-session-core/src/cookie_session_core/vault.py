...
class CookieVault:
    ...
    def encrypt(self, value: str, user_id: str, service_id: str, name: str) -> EncryptedValue:
        ...
        ciphertext = AESGCM(self._key(user_id, service_id)).encrypt(nonce, value.encode(), aad)
    ...
    def decrypt(
        ...
        aad = f"{user_id}:{service_id}:{name}".encode()
        return AESGCM(self._key(user_id, service_id)).decrypt(nonce, ciphertext, aad).decode()
