...
def escaped_proxy_service_id(
    ...
    if not browser_referer:
        return None
    referer = urlparse(browser_referer)
    if public_hostname and (referer.hostname or "").lower().rstrip(
        "."
    ) != public_hostname.lower().rstrip("."):
        return None
...
def _runtime_script(launch: ConsumedLaunch, prefix: str, public_base_url: str) -> str:
    ...
    return (
        ...
        "if(C.profilePrivacy&&C.profilePrivacy.enabled&&window.document&&document.querySelectorAll&&document.createElement){"
        "const selectors=C.profilePrivacy.selectors||[],sel=selectors.join(',');"
        "const inEditable=function(el){return !!el.closest('input,textarea,select,[contenteditable=\"true\"],form')};"
        "const candidate=function(t){if(!(t instanceof Element)||!sel)return null;"
        "let b=null;try{b=t.closest(sel)}catch(_e){b=null}return b&&!inEditable(b)?b:null};"
        "const blocked=function(e){return !!candidate(e.target)};"
        "const stop=function(e){const b=candidate(e.target);if(!b)return;e.preventDefault();e.stopPropagation();"
        "e.stopImmediatePropagation();if(b&&b.blur)b.blur()};"
        ...
        "style.textContent='.jbtools-profile-protected{cursor:not-allowed!important;position:relative!important;user-select:none!important}'"
        "+'.jbtools-profile-lock{display:grid!important;place-items:center!important;flex:0 0 auto!important;'"
        ...
        "+'.jbtools-profile-lock svg{width:15px!important;height:15px!important}'"
        "+'.jbtools-profile-protected:hover .jbtools-profile-lock{background:rgba(124,58,237,.2)!important}';"
        "(document.head||document.documentElement).appendChild(style);"
        "const controls=function(){const out=new Set();if(!sel)return out;"
        "try{document.querySelectorAll(sel).forEach(function(b){if(!inEditable(b))out.add(b)})}catch(_e){}return out};"
        "const lock=function(){controls().forEach(function(b){"
        "b.classList.add('jbtools-profile-protected');"
