...
def _expiry(value: object) -> datetime | None:
    if value in (None, "", 0, "0", "Session"):
        return None
    try:
        timestamp = float(cast(Any, value))
...
def validate_cookie(
    ...
    if not cookie.path.startswith("/") or not any(
        prefix == "/" or cookie.path == prefix or cookie.path.startswith(prefix.rstrip("/") + "/")
...
def validate_localstorage_key(key: object) -> str:
    if not isinstance(key, str):
        raise ValueError("LocalStorage key must be a string")
    if _utf8_size(key, "LocalStorage key") > 1_024:
        raise ValueError(f"LocalStorage key is larger than 1 KB: {key[:40]}")
    if "\x00" in key:
        raise ValueError("LocalStorage key cannot contain a null character")
    return key


def validate_localstorage_value(value: object) -> str:
    if not isinstance(value, str):
        raise ValueError("LocalStorage value must be a string")
    if _utf8_size(value, "LocalStorage value") > 512_000:
        raise ValueError("LocalStorage value is larger than 500 KB")
    return value


def parse_localstorage_import(raw: str) -> list[ImportedLocalStorageItem]:
    if not raw.strip() or _utf8_size(raw, "LocalStorage input") > 500_000:
        raise ValueError("LocalStorage input is empty or larger than 500 KB")
    try:
        decoded = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError("Invalid localStorage JSON") from exc
    if not isinstance(decoded, dict):
        raise ValueError("LocalStorage JSON must be a plain object with key-value pairs")
    output: list[ImportedLocalStorageItem] = []
    for key, value in decoded.items():
        key = validate_localstorage_key(key)
        str_value = value if isinstance(value, str) else json.dumps(value, ensure_ascii=False)
        validate_localstorage_value(str_value)
        output.append(ImportedLocalStorageItem(key=key, value=str_value))
    if not output:
        raise ValueError("LocalStorage import must contain at least one item")
    if len(output) > 2000:
        raise ValueError("LocalStorage import cannot exceed 2000 items")
    return output
